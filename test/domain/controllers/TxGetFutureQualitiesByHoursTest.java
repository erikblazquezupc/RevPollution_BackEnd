...
public class TxGetFutureQualitiesByHoursTest {
    DataCtrl db;
    QualityDataCtrl qdb;
    Quality q;
    StationStub s;
    ...
    @Test
    public void testTxGetFutureQualitiesByHours() {
        TxGetFutureQualitiesByHours tx = new TxGetFutureQualitiesByHours(s.getId());
        tx.execute();
        ...
        if (h < 14) {
            assertEquals(1, tx.getResult().size());
            assertEquals(200.0, tx.getResult().get(0).getValue(), 0);
        }
        else assertEquals(0, tx.getResult().size());
    }
}
