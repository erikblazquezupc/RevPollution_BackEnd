...
public class TxGetFutureQualitiesTest {
    ...
    @Test
    public void testTxGetFutureQualities() {
        TxGetFutureQualities tx = new TxGetFutureQualities(s.getId());
        tx.execute();
        assertNotNull(tx.getResult());
        assertEquals(tx.getResult().size(), 3);
        assertEquals(190.0, tx.getResult().get(0).getValue(), 0);
        assertEquals(180.0, tx.getResult().get(1).getValue(), 0);
        assertEquals(170.0, tx.getResult().get(2).getValue(), 0);
    }
}
